...
<AccordionGroup>
  ...
  <Accordion title="_write_telemetry">
    Iterates every lap in `session.laps`, pulling that lap's `get_telemetry()` individually. A single lap's `get_telemetry()` call is wrapped in its own inner `try/except`: an isolated lap with no telemetry is normal, so the write goes ahead, but every failure is counted. The writer returns a coverage record (laps attempted vs merged, how many drivers had car-data and position samples, the commonest errors) that goes into the run manifest; a race under 95% coverage is logged as `[LOW TELEMETRY]` unless it is a documented known gap (2018 Rd1/Rd2). What is written for a healthy race is unchanged.
  </Accordion>
  ...
  <Accordion title="_write_circuit_info">
    Writes corner geometry from `session.get_circuit_info()` one row per corner, with coordinates and marshal-sector numbers. A failure (e.g. MultiViewer has no layout for a new venue) is tolerated but returned as `failed: <error>` and recorded in the manifest's `circuit_info_status`, so a race without circuit info is visible without reading the log.
  </Accordion>
