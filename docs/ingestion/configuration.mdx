...
description: "The full ingestion config surface four optional environment variables, no credentials, and how the on-disk FastF1 cache works."
---

Ingestion needs no credentials. `EnvironmentConfig` exposes four settings, each with a sensible default, each overridable via an environment variable or a `.env` file.
...
<ParamField path="INGESTION_BRONZE_DIR" type="path" default="data/bronze">
  Bronze root `ingest.py` writes under (same layout), overridden per-run by `--bronze-dir`. Point it at a scratch directory for probes and experiments so they never write into `data/bronze`. `verify_bronze.py`, `manifest_report.py` and `scripts/check_season_seeds.py` take the same `--bronze-dir` flag.
</ParamField>
