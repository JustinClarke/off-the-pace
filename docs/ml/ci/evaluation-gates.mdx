...
The evaluation gates check `evaluation_metrics.json`; the model card (`model_card.yml`) is written by `ml-card` from the same source. The reference docs (`docs/reference/ml/degradation-model-v1.mdx`) are generated from the card. The chain is:
...
<CardGroup cols={2}>
  ...
  <Card title="CI overview" href="/ml/ci/overview" icon="vial">
    All 28 tests where these seven sit in the full contract.
  </Card>
</CardGroup>
