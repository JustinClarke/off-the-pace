...
<Note>
  Telemetry data for **2018 Rd1 and Rd2** is missing because F1's archive lacks the streams it is built from. For the Australian (Rd1) race the archive has the car-data stream (speed, throttle, brake) but no position stream, and FastF1 merges per-lap telemetry only when it has both; for the Bahraini (Rd2) race it has neither. Qualifying at both weekends has both streams, but the pipeline ingests race telemetry only. Laps, weather, and race control for those rounds are present. Telemetry coverage is complete from 2018 Rd3 onward.
</Note>
...
|---|---|---|
| Missing telemetry 2018 Rd1 and Rd2 (not in F1's archive; see Coverage above) | Telemetry only | No workaround. Exclude those rounds from telemetry-dependent analyses. |
