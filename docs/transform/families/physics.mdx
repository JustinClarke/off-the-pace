...
<Tabs>
  ...
  <Tab title="Other approaches">
    `int_lap_telemetry_aggregates`' five cliff-drift features are computed lap-internally (within-lap speed minima, brake-onset events) rather than joined against the circuit corner catalogue (`dim_corners`). The catalogue only resolves for a small fraction of circuits and is inconsistently named where it does; the lap-internal formulation trades a small loss of named-corner specificity for full telemetry coverage across every circuit, with missingness left explicit for the gradient-boosted model that reads it downstream rather than backfilled.
