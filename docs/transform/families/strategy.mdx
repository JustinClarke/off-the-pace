...
description: "Counterfactual, base-rate and stint-ending models built on top of the closed residual and the cliff prediction one already shipped to the app, others built ahead of any consumer."
---

## What this family does

Strategy answers decision-support questions the rest of the pipeline doesn't: was a pit stop early, late, or undercut-forced; how often does a given circuit see a Safety Car per racing lap; which constructors degrade their tyres faster or hit the cliff later than the field; and why a given stint ended at all. These are counterfactual, base-rate or labelling estimates built on top of earlier families, not new physics terms none of them feeds back into the residual identity.
...
flowchart LR
    ...
    int_stint_end_regime -->|is_censored_stint, stint_end_cause| fct_stint_features
    ...
    class int_constructor_deg_sensitivity,int_pit_strategy_value,int_sc_hazard_history,int_pit_loss_circuit,int_stint_end_regime strat;
...
<CardGroup cols={3}>
  ...
  <Card title="int_stint_end_regime" icon="flag-triangle-right" href="/reference/models/int/int_stint_end_regime">
    Why each stint ended, at stint grain: green_pit / sc_pit / vsc_pit / red / race_end / retirement, under a declared precedence. Owns is_censored_stint. 28.8% of the stints the survival target calls completed tyre lives ended under a deployment instead.
  </Card>
</CardGroup>
