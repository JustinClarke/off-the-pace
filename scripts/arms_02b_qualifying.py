"""02b — Tier 1 qualifying: the pre-registered arms.

Runs exactly the arms declared in _improvements/work/02-feature-expansion.md's `02b`
section before any of them was run, on the families that are not barred:
...
Usage:  python -m scripts.arms_02b_qualifying
        (or  PYTHONPATH=. ./.venv/bin/python scripts/arms_02b_qualifying.py)

Writes ml/artefacts/02b_qualifying_arms.json -- every fit's headline, per seed, so a
...
# ─── The seven columns, in the order the mart emits them ────────────────────────
COVERAGE_COL = ("quali_push_laps_n",)
CONSTRUCTOR_PACE_COLS = (
...
ARMS: dict[str, tuple[str, ...]] = {
    # A carries all seven, including quali_push_laps_n -- the leaf doc's 02b section
    # names why that column is not split into its own confound arm the way 02c's
    # corner_input_coverage was: it is a perfect indicator of the other six columns'
    # missingness (verified NULL iff quali_push_laps_n == 0, both directions), not a
    # continuous channel with a plausible independent signal of its own.
    "A_full": COVERAGE_COL + CONSTRUCTOR_PACE_COLS + DRIVER_FORM_COLS,   # 7
    "B_constructor_pace": CONSTRUCTOR_PACE_COLS,                        # 2
    "C_driver_form": DRIVER_FORM_COLS,                                  # 4
}
...
OUT = Path("ml/artefacts/02b_qualifying_arms.json")


# ─── Column plumbing ────────────────────────────────────────────────────────────
def qualifying_frame(duckdb_path: str = S.DUCKDB_PATH) -> pd.DataFrame:
    """The seven mart columns, indexed by lap_id, float32, NaN preserved.
    ...
    them. The mart has already resolved the (race_year, race_id, driver_id) join and
    broadcast the weekend-grain values onto every lap, so a lap_id read here needs no
    further join. Encoding matches `_encode_frame`'s continuous branch exactly --
    to_numeric then float32, native NaN left as NaN (quali_push_laps_n is never NaN;
    it is COALESCEd to 0 in the mart).
    """
    cols = ", ".join(COVERAGE_COL + CONSTRUCTOR_PACE_COLS + DRIVER_FORM_COLS)
...
def attach(X: pd.DataFrame, lap_ids: np.ndarray, extra: pd.DataFrame,
    ...
    if block.isna().all(axis=1).all():
        raise RuntimeError("no qualifying rows matched the split's lap_ids")
    out = X.copy()
    for c in cols:
...
def shuffled(X: pd.DataFrame, cols: tuple[str, ...], rng: np.random.Generator) -> pd.DataFrame:
    ...
    with their values, same caveat 02c's version of this function names.
    """
    out = X.copy()
...
# ─── E-value, Construction B (paired safe-t) -- identical to 02c's implementation ──
...
def run_family(target: str, split, extra: pd.DataFrame, published: float,
    ...
    res["negative_control_shuffle_vs_shuffle"] = {
        ...
        "note": ("both arms are shuffles of the same seven columns, so H0 is true by "
...
def main() -> int:
    ...
    extra = qualifying_frame()
    log(f"qualifying columns loaded: {extra.shape[0]} lap_ids x {extra.shape[1]} columns")
