-- Constructor structural pace model: Constructor structural pace via
-- aggregation.
--
-- Estimates the constructor coefficient using within-constructor,
-- between-driver
-- variation. This implementation uses grouped statistics as a placeholder for
-- the
...
-- (correction_weight) and int_track_evolution (rainfall_flag) instead of
-- int_lap_anomaly_flags.
-- The anomaly_class filter (mistake/conditions) is approximated by
-- correction_weight = 1.0,
-- which catches the same SC/VSC/outlier laps; fine-grained mistake detection is
-- downstream.
--
-- Subsequent integration: replace aggregation with pyfixest HDFE panel
-- regression
...
clean_panel AS (
    SELECT
        ...
        f.driver_id,
        lm.constructor_id,
        -- Partial pace delta (vs field median baseline), before component
        -- subtraction
        f.lap_time_s
...
-- The raw median_pace_delta conflates driver skill with car pace. Subtracting
-- the
-- race mean produces a zero-summing relative coefficient: positive = slower
-- than
-- average constructor, negative = faster. This is the correct interpretation
-- per
...
SELECT
    ...
    ca.race_id,
    ca.constructor_id,
    -- Re-centred coefficient: constructor pace relative to the race-average
    -- constructor.
