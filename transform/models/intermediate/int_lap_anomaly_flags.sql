...
--   'clean_cliff' -statistically extreme residual consistent with tyre cliff
--   pattern
--   'mistake'     -large positive residual spike not explained by cliff (driver
--   error or damage)
--   'event_driven' lap contaminated by a controllable race event (SC, VSC,
--   restart, yellow)
--   'conditions'  -rain lap or ambient condition anomaly
--   'normal'      -no anomaly detected
--
-- Scale estimator: trailing-7-lap MAD (Median Absolute Deviation) per driver ×
-- race,
-- floored at 0.10s. MAD is robust to the cliff itself; stddev is contaminated.
-- residual_z_score is retained for traceability but anomaly_class uses
...
trailing_window AS (
    SELECT
        ...
        -- Trailing median over laps [lap_number-6, lap_number] (inclusive,
        -- trailing-only)
    ...
    SELECT
        ...
        CASE
            WHEN rs.stddev_residual_s > 0
                THEN
                    (r.driver_skill_residual_s - rs.mean_residual_s)
                    / rs.stddev_residual_s
            ELSE 0.0
        ...
        CASE
            WHEN tm.mad_floored_s > 0
                THEN
                    ABS(r.driver_skill_residual_s - tm.trailing_median_s)
                    / tm.mad_floored_s
            ELSE 0.0
        END AS mad_score
...
SELECT
    ...
    -- Composite anomaly class (spec values: clean_cliff, mistake, event_driven,
    -- conditions, normal)
    CASE
        ...
        WHEN
            ...
            THEN 'clean_cliff'
        -- mistake: large positive spike NOT explained by cliff (driver error,
        -- damage, lock-up)
