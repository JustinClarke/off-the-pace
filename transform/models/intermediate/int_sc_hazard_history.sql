-- int_sc_hazard_history.sql · intermediate · grain: one row per circuit (venue slug)
...
-- interruption on each simulated lap. Estimated from the SC/VSC deployment events
-- in stg_track_status, with racing-lap exposure from stg_laps.
--
-- Scope: only races that have an ingested track_status timeline (partial during the
-- v0.2 backfill). Onset and exposure are taken over the SAME race set so the rate is
-- well defined. The venue key is the race name slug (e.g. monaco_grand_prix), which
-- recurs across seasons; stg_laps.race_id is the numeric per-season id, so we map it
...
-- SC/VSC deployment ONSETS per race. status_code 4 = SCDeployed, 6 = VSCDeployed
...
onsets AS (
    ...
    FROM {{ ref('stg_track_status') }} ts
    GROUP BY ts.race_year, ts.race_id
),

-- Racing-lap exposure per race: the leader's lap count ≈ race distance, i.e. the
...
per_race AS (
    SELECT
        ...
        e.racing_laps
    FROM races_in_scope s
    LEFT JOIN onsets o   USING (race_year, race_id)
    JOIN exposure e      USING (race_year, race_id)
...
global_rate AS (
    SELECT
        CAST(SUM(n_sc_onsets)  AS DOUBLE) / NULLIF(SUM(racing_laps), 0) AS sc_rate,
        CAST(SUM(n_vsc_onsets) AS DOUBLE) / NULLIF(SUM(racing_laps), 0) AS vsc_rate,
        CAST(SUM(n_any_onsets) AS DOUBLE) / NULLIF(SUM(racing_laps), 0) AS any_rate
    FROM per_circuit
)
...
SELECT
    ...
    -- {{ var('sc_hazard_prior_laps', 600) }} laps of prior weight (≈ a few races'
    ...
    (c.n_sc_onsets  + g.sc_rate  * {{ var('sc_hazard_prior_laps', 600) }})
        / (c.racing_laps + {{ var('sc_hazard_prior_laps', 600) }}) AS sc_hazard_per_lap_shrunk,
    (c.n_vsc_onsets + g.vsc_rate * {{ var('sc_hazard_prior_laps', 600) }})
        / (c.racing_laps + {{ var('sc_hazard_prior_laps', 600) }}) AS vsc_hazard_per_lap_shrunk,
    (c.n_any_onsets + g.any_rate * {{ var('sc_hazard_prior_laps', 600) }})
        / (c.racing_laps + {{ var('sc_hazard_prior_laps', 600) }}) AS any_hazard_per_lap_shrunk
FROM per_circuit c
