...
-- yet).
-- Subsequent integration: when int_constructor_structural_pace gains
-- power_loading_s / aero_loading_s,
-- update allocation: power → S1+S3, aero → S2.
--
-- NOTE: dirty_air_share_lap is lap-level only (no per-sector breakdown
-- available).
...
--   For each physics component C (fuel, tyre deg, weather, etc.), we allocate
--   it
--   proportionally to a sector's time share:
--     sector_component = lap_component * (sector_time / lap_time)
-- 
--   Since Sum_{s=1}^{3} (sector_time / lap_time) = 1.0, this mathematical
--   property ensures
--   that the sum of the sector-grain components exactly equals the lap-grain
--   component:
--     Sum_{s=1}^{3} sector_component = lap_component
-- 
--   Note on sector_pace_delta_s and field baselines:
--     sector_pace_delta_s is computed as (sector_time_s
--     field_sector_pace_smoothed_s),
--     where field_sector_pace_smoothed_s is based on sector medians. Because
--     the median of a sum
--     does not equal the sum of medians, the sector baseline SUM does not
--     perfectly equal the lap
--     trimmed-mean baseline. The driver sector residual captures this
--     difference.
allocated AS (
    SELECT
        *,
