...
-- For each ego driver × lap, finds the teammate (same constructor, different
-- driver)
...
joined AS (
    SELECT
        ...
        -- compound/age
        t.tm_wc_lap_time_s
        + (e.ego_compound_pace_s - t.tm_compound_pace_s)
            AS teammate_pace_adjusted_s,
        ...
        *,
        ...
        1.0 / (1.0 + COALESCE(
            STDDEV(driver_skill_proxy_s) OVER (
                PARTITION BY
                    race_year, race_id, ego_driver_id, teammate_driver_id
                ORDER BY lap_number
