-- Gold layer: driver-race skill feature table for driver skill extraction
-- model.
-- Grain: (race_year, race_id, driver_id) one row per driver per race.
-- Aggregates lap-level residuals, synthetic teammate deltas, constructor pace
-- indices,
...
--
-- driver_skill_residual_s sign convention: negative = faster than field (after
-- physics removal).
-- driver_skill_proxy_s sign convention: positive = ego faster than synthetic
-- teammate.
...
teammate_agg AS (
    SELECT
        ...
        ROUND(
            CAST(
                COUNT(
                    CASE WHEN st.strategic_divergence_flag THEN 1 END
                ) AS DOUBLE
            )
            / NULLIF(COUNT(*), 0),
            4
...
SELECT
    {{ dbt_utils.generate_surrogate_key([
        'ra.race_year', 'ra.race_id', 'ra.driver_id'
    ]) }}
        AS driver_race_id,
    ra.driver_id,
    ra.race_year,
