...
--                      + fuel_component          (circuit/race, not
--                      driver-specific)
--                      + driver_skill_residual    (ego driver's actual
--                      residual)
...
-- laps_past_cliff).
-- cliff_onset_shift moves each constructor's effective onset earlier (negative)
-- or
-- later (positive). This term only fires once tyres approach/pass the cliff, so
-- it
-- reorders drivers on long stints near the end of their tyre life where the
-- linear
...
-- The self-consistency test asserts this holds within 0.0001 s. Both
-- interaction
-- terms preserve it exactly: every lookup hits the same cell, so each
-- difference
...
-- as the ego driver lap. Rows where recombination_confidence < 0.05 are
-- excluded.
...
WITH ego_laps AS (
    ...
    -- Accept clean (1.0) and soft-down-weighted (0.6 yellow / soft outlier)
    -- laps so
    -- partial-race drivers retain a usable sample; the weight feeds lap
    -- confidence below.
...
-- Cartesian: every (ego_driver_race_lap) × (valid host_constructor in same
-- race)
ghost_recombined AS (
    SELECT
        MD5(CONCAT(
        ...
        COALESCE(hcp.constructor_structural_pace_s, 0.0)
            AS host_constructor_pace_s,
        COALESCE(hcp.constructor_structural_pace_se_s, 0.0)
        ...
        -- per-lap
        -- exposures (tyre age, field severity, and whether each car's hinge is
        -- active)
        -- that scale those coefficient uncertainties into the predicted-pace
        -- variance.
        COALESCE(deg_host.deg_slope_posterior_sd_s_per_lap, 0.0)
            AS host_deg_slope_sd_s_per_lap,
        COALESCE(deg_ego.deg_slope_posterior_sd_s_per_lap, 0.0)
            AS ego_deg_slope_sd_s_per_lap,
        COALESCE(deg_host.cliff_onset_shift_se_laps, 0.0)
            AS host_cliff_shift_se_laps,
        COALESCE(deg_ego.cliff_onset_shift_se_laps, 0.0)
            AS ego_cliff_shift_se_laps,
        ...
        -- 1.0 when this car is past its (shifted) cliff onset on this lap, else
        -- 0.0.
        -- Averaged downstream = fraction of laps the cliff term is active for
        -- the car,
        ...
        --   host_obs_factor : shrinkage on how well the host car's pace is
        --   estimated
        --                     panel_obs / (panel_obs + 50) → smoothly 0→1
        --   lap_quality     : the lap's own correction_weight (1.0 clean, 0.6
        --   soft)
        -- Scenario-level coverage weighting is applied downstream in
        -- fct_ghost_race_finish.
        (
    ...
    LEFT JOIN host_constructor_pace AS hcp
        ON
            el.race_year = hcp.race_year
            AND el.race_id = hcp.race_id
            AND hc.host_constructor_id = hcp.host_constructor_id
    LEFT JOIN host_interaction AS hi
        ON
            el.race_year = hi.race_year
            AND el.race_id = hi.race_id
            AND hc.host_constructor_id = hi.host_constructor_id
    LEFT JOIN deg_sensitivity AS deg_host
        ON
            el.race_year = deg_host.race_year
            AND hc.host_constructor_id = deg_host.constructor_id
            AND el.compound = deg_host.compound
    LEFT JOIN deg_sensitivity AS deg_ego
        ON
            el.race_year = deg_ego.race_year
