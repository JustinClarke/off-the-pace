-- Gold layer: lap-grain feature table for machine learning (XGBoost degradation
-- model).
-- Thin wrapper over int_lap_residual_decomposed that adds:
--   • ml_eligible flag -rows the model should train on (full weight, compound
--   known)
...
SELECT
    ...
    -- and component blowups (compound > 10s or |constructor| > 10s) from
    -- thin samples.
