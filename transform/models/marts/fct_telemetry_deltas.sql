-- fct_telemetry_deltas.sql · marts · grain: one row per driver × lap × circuit
-- turn
...
teammates AS (
    ...
    FROM laps_info AS l1
    INNER JOIN laps_info AS l2
        ON
            l1.race_id = l2.race_id
...
deltas AS (
    ...
    LEFT JOIN metrics AS m_a
        ON
            tm.race_id = m_a.race_id
            AND tm.driver_a = m_a.driver_id
    LEFT JOIN metrics AS m_b
        ON
            tm.race_id = m_b.race_id
