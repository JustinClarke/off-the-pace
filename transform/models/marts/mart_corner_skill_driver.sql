...
--    instead of raw braking/throttle geometry deviations, which encoded racing-line
--    style differences as unskilled (e.g. Hamilton's earlier braking approach).
-- 2. LORO car baseline. The car's corner effect is the mean residual of the OTHER
--    same-car drivers at the same race and corner (excluding the focal driver). The
--    old model included the focal driver in the car average, so only ~half the edge
--    over a teammate survived.
-- 3. Push-lap filter (clean panel). Applies the correction_weight=1.0 and
--    rainfall_flag=FALSE filters to exclude yellow flags, restarts, and wet laps
--    from the car baseline, matching the clean panel of int_driver_race_skill_loro.
--
...
-- Corner residuals restricted to push / clean laps (no rain, standard validity flags,
-- no yellow-flag / restart corrections). All residuals already "seconds vs field median".
push_corners AS (
    SELECT
        csr.race_year,
        csr.race_id,
        ...
        csr.exit_residual_s
    FROM {{ ref('int_corner_skill_residuals') }} csr
    JOIN lap_meta lm                                           USING (lap_id)
    LEFT JOIN corrections cor                                  USING (lap_id)
    LEFT JOIN evolution e
        ON  csr.race_year  = e.race_year
        AND csr.race_id    = e.race_id
        AND csr.lap_number = e.lap_number
    WHERE NOT csr.corner_unmapped_flag
      AND lm.is_accurate
...
-- Per-(race, constructor, corner): sum and count of residuals across ALL drivers
...
-- Per-(race, constructor, driver, corner): focal driver's summed residual and count
-- (summed because the driver may have several valid laps at this corner in one race).
...
-- equal-car reference → corner drops out of the aggregation, same as NULL residual).
loro_corner AS (
    SELECT
        dca.race_year,
        dca.race_id,
        ...
        CASE
            ...
            THEN (ca.sum_braking_s - COALESCE(dca.driver_sum_braking_s, 0))
                 / NULLIF(ca.n_braking - dca.n_braking, 0)
        END AS loro_braking_s,
        CASE
            WHEN ca.n_mid > dca.n_mid
            THEN (ca.sum_mid_s - COALESCE(dca.driver_sum_mid_s, 0))
                 / NULLIF(ca.n_mid - dca.n_mid, 0)
        END AS loro_mid_s,
        CASE
            WHEN ca.n_exit > dca.n_exit
            THEN (ca.sum_exit_s - COALESCE(dca.driver_sum_exit_s, 0))
                 / NULLIF(ca.n_exit - dca.n_exit, 0)
        END AS loro_exit_s,
        dca.driver_sum_braking_s / NULLIF(dca.n_braking, 0) AS driver_mean_braking_s,
    ...
    FROM driver_corner_agg dca
    JOIN car_agg ca
        ON  dca.race_id        = ca.race_id
...
deconf_corner AS (
    ...
    FROM loro_corner
    WHERE loro_braking_s IS NOT NULL
       OR loro_mid_s     IS NOT NULL
       OR loro_exit_s    IS NOT NULL
),

-- Season-driver aggregates: mean LORO-adjusted skill across all corners and races.
-- Require ≥ 100 mapped corners for reliability (same guard as the original model).
driver_season AS (
    SELECT
        race_year,
        driver_id,
...
-- Per-phase z-scores within each season so all three phases contribute equally to
-- corner_skill_index and the exit phase does not dominate (unchanged from old model).
standardized AS (
    SELECT
        race_year,
        driver_id,
        ...
        exit_skill_s,
        mapped_corners,
        braking_skill_s    / NULLIF(STDDEV(braking_skill_s)    OVER (PARTITION BY race_year), 0) AS braking_skill_z,
        mid_corner_skill_s / NULLIF(STDDEV(mid_corner_skill_s) OVER (PARTITION BY race_year), 0) AS mid_corner_skill_z,
        exit_skill_s       / NULLIF(STDDEV(exit_skill_s)       OVER (PARTITION BY race_year), 0) AS exit_skill_z
    FROM driver_season
)
...
SELECT
    ...
    ROUND(braking_skill_z + mid_corner_skill_z + exit_skill_z, 2) AS corner_skill_index,
    mapped_corners
FROM standardized
ORDER BY race_year, corner_skill_index ASC
