...
-- Lifts the circuit_reference seed into a typed dimension: lap length, corner
-- count,
-- lateral-g proxy, and weight-penalty coefficient fitted by
-- tasks/coefficients/.
--
-- circuit_key is a grand-prix/event slug, so several keys can share one
-- physical venue
-- (renamed events, double-headers). circuit_id is the stable physical-circuit
-- identifier:
...
SELECT
    ...
    CAST(fuel_consumption_rate_kg_per_lap AS DOUBLE)
        AS fuel_consumption_rate_kg_per_lap,
