-- stg_results.sql · staging · grain: one row per driver × race (classified result)
-- Renames FastF1 session.results to snake_case, casts nanoseconds → seconds, and
-- derives an explicit DNF / classification split (replacing the MAX(position)
-- finish derivation downstream see transform-v0.2 §5.4). No joins, no aggregations.
...
SELECT
    ...
    -- race_id is the numeric FastF1 event id (e.g. 2018_4) matches stg_laps.race_id.
    ...
    CASE WHEN Time IS NOT NULL THEN CAST(Time AS DOUBLE) / 1e9 ELSE NULL END AS time_or_gap_s,

    -- Raw status string ('Finished', '+1 Lap', 'Engine', 'Collision', ...)
    CAST(Status AS VARCHAR) AS status,

    -- Classification flags.
    -- ClassifiedPosition is numeric for ranked finishers; 'R'/'D'/'W'/'E'/'F'/'N'
    -- for retired/disqualified/withdrawn/etc. A driver is "classified" (counts as
    ...
    CASE
        ...
        WHEN REGEXP_MATCHES(LOWER(Status), 'collision|accident|crash|spun|spin|contact|damage') THEN 'racing'
        WHEN REGEXP_MATCHES(LOWER(Status), 'disqualif|withdr|did not') THEN 'non_classified'
        ELSE 'mechanical'
    END AS dnf_cause
