...
-- Apply the same min_observations=1 floor the model uses. This is a SECOND COPY
-- of that parameter (08i flagged it as such): it must move with any floor change
-- in int_lap_thermal_proxy.sql or this test fires spuriously on every row in the
-- gap between the two floors.
prior_agg AS (
    SELECT
        cur.lap_id,
        CASE
            WHEN COUNT(prv.lap_time_s) FILTER (WHERE prv.is_valid_lap) >= 1
...
-- floor check: the baseline is NULL exactly where fewer than 1 valid prior lap
...
WHERE t.baseline_observations_n IS DISTINCT FROM p.expected_n_prior
   OR (t.stint_baseline_pace IS NULL) <> (p.expected_n_prior < 1)
